...
<Modal>
  <main class="2xl:w-3/12 xl:w-4/12 lg:w-6/12 md:w-8/12 sm:w-8/12 bg-white p-8 m-auto rounded mt-20 height-auto">
    ...
    <div class="w-full max-w-md space-y-4 flex items-center justify-between">
      <div class="flex-col">
        <h2 class="font-sans text-left text-2xl font-light tracking-tight text-gray-400">Save to collection(s)</h2>
      </div>
    ...
    <form class="mt-8 space-y-6" on:submit={onSubmit}>
      ...
      <Button text={`Update collection${toggledCollectionIds.length > 1 ? 's' : ''}`} type="submit" {inProgress} />
    </form>
  </main>
</Modal>
