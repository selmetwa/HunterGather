...
<div class="my-8 sm:px-8 md:px-16 xl:px-24 flex justify-between items-center flex-col md:flex-row" transition:fade>
	{#if isSearchPage}
		<h1 class="font-sans text-left text-3xl font-light tracking-tight text-gray-400 my-6">
			{!!objectType ? `${count} ${objectType} matching` : `${count} search results for`} <span class="text-action-400">{searchTerm}</span>
		</h1>
		<div>
